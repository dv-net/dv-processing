...
func NewFSM(
	...
	if conf.Blockchain.BitcoinCash.Attributes.FeePerByte > 0 {
		fsm.feePerByte = decimal.NewFromInt(conf.Blockchain.BitcoinCash.Attributes.FeePerByte)
	}

	if conf.Blockchain.BitcoinCash.Attributes.MinUTXOAmount > 0 {
		fsm.minUTXOAmount = decimal.NewFromInt(conf.Blockchain.BitcoinCash.Attributes.MinUTXOAmount)
	}

	// create a workflow
	fsm.wf = workflow.New(
		workflow.WithName("BCH FSM"),
...
func (s *FSM) validateRequest(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	...
	if s.transfer.WalletFromType == constants.WalletTypeHot {
		checkResult, err := s.bs.Wallets().CheckWallet(ctx, wconstants.BlockchainTypeBitcoinCash, s.transfer.GetToAddress())
		if err != nil {
...
func (s *FSM) sendTransfer(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	...
	if s.config.Blockchain.BitcoinCash.Network == "testnet" {
	...
	newTx := bch.NewTxBuilder(s.bch.WalletSDK.ChainParams())
	...
	// send transaction
	if _, err := s.bch.Node().SendRawTransaction(newTx.MsgTx(), false); err != nil {
		return fmt.Errorf("failed to send transaction [%s]: %w", newTx.MsgTx().TxHash().String(), err)
	}
...
func (s *FSM) waitingInMempool(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	txs, err := s.bs.EProxy().FindTransactions(ctx, wconstants.BlockchainTypeBitcoinCash, eproxy.FindTransactionsParams{
...
func (s *FSM) waitingForTheFirstConfirmation(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	tx, err := s.bs.EProxy().GetTransactionInfo(ctx, wconstants.BlockchainTypeBitcoinCash, s.transfer.TxHash.String)
	if err != nil {
	...
	if tx.Confirmations == 0 {
		delay := constants.ConfirmationsTimeout(wconstants.BlockchainTypeBitcoinCash, constants.GetMinConfirmations(wconstants.BlockchainTypeBitcoinCash)-1)
...
func (s *FSM) waitingConfirmations(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	...
	delay := constants.ConfirmationsTimeout(wconstants.BlockchainTypeBitcoinCash, tx.Confirmations)
	minConfirmationsCount := constants.GetMinConfirmations(wconstants.BlockchainTypeBitcoinCash)
