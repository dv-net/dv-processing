...
func NewFSM(
	...
	if conf.Blockchain.Dogecoin.Attributes.FeePerByte > 0 {
		fsm.feePerByte = decimal.NewFromInt(conf.Blockchain.Dogecoin.Attributes.FeePerByte)
	}

	if conf.Blockchain.Dogecoin.Attributes.MinUTXOAmount > 0 {
		fsm.minUTXOAmount = decimal.NewFromInt(conf.Blockchain.Dogecoin.Attributes.MinUTXOAmount)
	}

	// create a workflow
	fsm.wf = workflow.New(
		workflow.WithName("DOGECOIN FSM"),
...
func (s *FSM) validateRequest(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	...
	if s.transfer.WalletFromType == constants.WalletTypeHot {
		checkResult, err := s.bs.Wallets().CheckWallet(ctx, wconstants.BlockchainTypeDogecoin, s.transfer.GetToAddress())
		if err != nil {
...
func (s *FSM) sendTransfer(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	...
	if s.config.Blockchain.Dogecoin.Network == "testnet" {
	...
	newTx := doge.NewTxBuilder(s.doge.WalletSDK.ChainParams())
	...
	go func() {
		_, err := s.doge.Node().SendRawTransaction(newTx.MsgTx(), false)
		if err != nil {
...
func (s *FSM) waitingInMempool(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	txs, err := s.bs.EProxy().FindTransactions(ctx, wconstants.BlockchainTypeDogecoin, eproxy.FindTransactionsParams{
...
func (s *FSM) waitingForTheFirstConfirmation(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	tx, err := s.bs.EProxy().GetTransactionInfo(ctx, wconstants.BlockchainTypeDogecoin, s.transfer.TxHash.String)
	if err != nil {
	...
	if tx.Confirmations == 0 {
		delay := constants.ConfirmationsTimeout(wconstants.BlockchainTypeDogecoin, constants.GetMinConfirmations(wconstants.BlockchainTypeDogecoin)-1)
...
func (s *FSM) waitingConfirmations(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	...
	delay := constants.ConfirmationsTimeout(wconstants.BlockchainTypeDogecoin, tx.Confirmations)
	minConfirmationsCount := constants.GetMinConfirmations(wconstants.BlockchainTypeDogecoin)
