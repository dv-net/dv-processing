...
func NewFSM(
	...
	if conf.Blockchain.Litecoin.Attributes.FeePerByte > 0 {
		fsm.feePerByte = decimal.NewFromInt(conf.Blockchain.Litecoin.Attributes.FeePerByte)
	}

	if conf.Blockchain.Litecoin.Attributes.MinUTXOAmount > 0 {
		fsm.minUTXOAmount = decimal.NewFromInt(conf.Blockchain.Litecoin.Attributes.MinUTXOAmount)
	}

	// create a workflow
	fsm.wf = workflow.New(
		workflow.WithName("LTC FSM"),
...
func (s *FSM) validateRequest(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	...
	if !s.transfer.WholeAmount {
		return fmt.Errorf("only whole amount is supported for litecoin transfers")
	}

	// check cold or processing wallet
	if s.transfer.WalletFromType == constants.WalletTypeHot {
		checkResult, err := s.bs.Wallets().CheckWallet(ctx, wconstants.BlockchainTypeLitecoin, s.transfer.GetToAddress())
		if err != nil {
...
func (s *FSM) sendTransfer(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	...
	if s.config.Blockchain.Litecoin.Network == "testnet" {
	...
	newTx := ltc.NewTxBuilder(s.ltc.WalletSDK.ChainParams())
	...
	go func() {
		_, err := s.ltc.Node().SendRawTransaction(newTx.MsgTx(), false)
		if err != nil {
...
func (s *FSM) waitingInMempool(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	txs, err := s.bs.EProxy().FindTransactions(ctx, wconstants.BlockchainTypeLitecoin, eproxy.FindTransactionsParams{
...
func (s *FSM) waitingForTheFirstConfirmation(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	tx, err := s.bs.EProxy().GetTransactionInfo(ctx, wconstants.BlockchainTypeLitecoin, s.transfer.TxHash.String)
	if err != nil {
	...
	if tx.Confirmations == 0 {
		delay := constants.ConfirmationsTimeout(wconstants.BlockchainTypeLitecoin, constants.GetMinConfirmations(wconstants.BlockchainTypeLitecoin)-1)
...
func (s *FSM) waitingConfirmations(ctx context.Context, _ *workflow.Workflow, _ *workflow.Stage, _ *workflow.Step) error {
	...
	delay := constants.ConfirmationsTimeout(wconstants.BlockchainTypeLitecoin, tx.Confirmations)
	minConfirmationsCount := constants.GetMinConfirmations(wconstants.BlockchainTypeLitecoin)
