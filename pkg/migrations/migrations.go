...
func (s *Migration) prepare(ctx context.Context) (*migrate.Migrate, database.Driver, error) {
	...
	var params sql.MigrationParameters
	var driver database.Driver
	var err error

	if s.config.DBDriver != DBDriverPostgres {
		return nil, nil, fmt.Errorf("unsupported database driver: %s", s.config.DBDriver)
	}
	...
	m, err := migrate.NewWithInstance("iofs", sourceDriver, s.config.DBName, driver)
	if err != nil {
		return nil, nil, err
	}
	m.Log = s.logger
...
func (s *Migration) Down(ctx context.Context, steps int) error {
	...
	if steps == -1 {
		if err := m.Down(); err != nil {
			return fmt.Errorf("failed to rollback all migration: %w", err)
		}
		return nil
	}
	...
	if err := m.Steps(steps * -1); err != nil {
		return fmt.Errorf("failed to rollback %d migrations: %w", steps, err)
	}
