...
func (s WalletSDK) GenerateAddress(addressType AddressType, mnemonic, passphrase string, sequenceNumber uint32) (*GenerateAddressData, error) {
	...
	coinKey, err := purposeKey.Derive(hdkeychain.HardenedKeyStart + 2)
	if err != nil {
	...
	case AddressTypeP2SH: // SegWit (nested segwit) address
		pubKeyHash := ltcutil.Hash160(pubKey.SerializeCompressed())
		witAddr, err := ltcutil.NewAddressWitnessPubKeyHash(pubKeyHash, s.chainParams)
		if err != nil {
		...
		addr, err = ltcutil.NewAddressScriptHash(redeemScript, s.chainParams)
		if err != nil {
			return nil, fmt.Errorf("failed to create P2SH address: %w", err)
		}
	case AddressTypeP2WPKH: // Native SegWit (Bech32) address
		pubKeyHash := ltcutil.Hash160(pubKey.SerializeCompressed())
		addr, err = ltcutil.NewAddressWitnessPubKeyHash(pubKeyHash, s.chainParams)
		if err != nil {
	...
	case AddressTypeP2TR: // Taproot address
		...
		addr, err = ltcutil.NewAddressTaproot(tweakedPubKeyHash, s.chainParams)
		if err != nil {
			return nil, err
		}
	default:
